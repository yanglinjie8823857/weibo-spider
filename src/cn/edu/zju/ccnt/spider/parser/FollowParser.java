...
public class FollowParser {
	...
	public static Follow parse(Element followEl, String followerID, int currentLevel){
		...
		try {
			...
			for(Element e: followEl.getElementsByTag("td")){
...
//				}
				String f=e.getElementsByTag("a").get(0).attr("href");
				Log.info(f);
				String followurl=f.substring(16);
				follow.setFollower(f);
				follow.setFollowee(followurl);
				follow.setLevel(1);
			}
