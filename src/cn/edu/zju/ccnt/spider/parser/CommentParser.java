...
public class CommentParser {	
	...
	public static Comment parse(Element commentEl, String weiboID){
		...
		try {
			comment.setId(weiboID);
			
			//一部分人的ID并不是数字串，而是个性域名，这部分也尚待处理
			//如：/u/123245 和 /kaifulee
			String tempAuthor = commentEl.getElementsByAttribute("href").get(0).text();
			Log.info(tempAuthor);
			comment.setAuthor(tempAuthor);
			
			//获取一条评论内的有效内容，包括@的人
			String tempContent = commentEl.getElementsByClass("ctt").get(0).text();
...
//		Connection conn = DBConn.getConnection();
		...
		try{
			...
			for(Element e:commentItems){
				Comment comment = CommentParser.parse(e, weiboID);
