...
public class RepostParser {
	...
	public static Repost parse(Element repostEl, String weiboID){
		...
		try {
			...
			String tempAuthor = repostEl.getElementsByAttribute("href").get(0).text();
			repost.setAuthor(tempAuthor);
            ...
            //获取时间
//			Log.info(Utils.parseDate(repostEl.getElementsByClass("ct").get(0).text().split("来自")[0]).substring(1));
			repost.setTime(Utils.repostparseDate(repostEl.getElementsByClass("ct").get(0).text().split("来自")[0].trim()).trim());
		...
		try{
			...
			for(int i = 0; i < repostItems.size(); i++){
				...
				if(repost != null){
					...
					ps.setString(4, repost.getTime().replace("?", ""));
					ps.execute();
