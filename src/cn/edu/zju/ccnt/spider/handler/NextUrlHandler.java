...
public class NextUrlHandler {
	 ...
	 * @throws Exception 
	 */
	public  static  String addNextWeiboUrl(Page page) throws Exception{
			...
			if(pageEl != null){
				...
				for(Element el: hrefEls){
					if(el.toString().contains("下页")){
						WeiboUrlQueue.addFirstElement("http://weibo.cn" + el.attr("href").split("&gsid=")[0]);
		...
		if(pageEl != null){
			...
			for(Element el: hrefEls){
				if(el.toString().contains("下页")){
					...
					String hrefParts = el.attr("href");
					
					
					String nextUrl = "http://weibo.cn" + hrefParts;
