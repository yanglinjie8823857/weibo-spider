...
public class UrlFollowWorker extends BasicWorker implements Runnable {
	 ...
	 * 返回值：被封账号/系统繁忙/OK
	 */
	protected String dataHandler(String url,String cookie){
		return null;
	}
	
	protected String dataHandler(String url, CookieStore cookie, int currentLevel){
...
//				Log.info(">> All followees of all followers have been fetched...");
				...
				while(!FollowUrlQueue.isEmpty()) {
					...
					// 获取这个ID对应的所有followee，并存入数据库
					result = dataHandler1(followUrl, cookie, CURRENT_LEVEL);
	...
	public String dataHandler1(String url, CookieStore cookie,int currentLevel )
			throws IOException {
